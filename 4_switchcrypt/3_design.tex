...
\figref{sc-overview}, which provides an overview of the SwitchCrypt system
design.
...
upon initialization, illustrated in \figref{sc-backstore}. The HEAD consists of
...
\tblref{sc-security-quant}). Our framework is composed of three features: relative
...
very useful (see: \secref{sc-usecases}). The near-instantaneous nature of SSD
...
\secref{sc-usecases} for specific scenarios that highlight the practical difference
between strategies.
...
(see: \secref{sc-usecases}).

\subsection{Putting It All Together} \label{subsec:summary}

We revisit the motivating example from \secref{sc-motivation}. Initially, I/O
