...
\secref{sc-motivation}, demonstrating that the ability to re-cipher individual
...
4KB, 5MB Performance}\par\medskip
   \centering
   {\input{data/sc/usecase-vsr-bar.tex}} \caption{Median sequential read and
...
\begin{figure}[ht] \textbf{SSD EoL Use Case: Latency-Security Tradeoff vs
   Goals}\par\medskip
   {\input{data/sc/usecase-eol-tradeoff.tex}} \caption{Median sequential and
...
\begin{figure}[ht] \textbf{Custody Panic Use Case: Security Goals vs Time}\par\medskip
   \centering
   {\input{data/sc/usecase-custody.tex}} \caption{Actual security score vs
